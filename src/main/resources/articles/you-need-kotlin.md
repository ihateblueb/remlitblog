...
Anyway, this illustrates my point that I love this language and know a lot about it. This post serves to host an overview of
my current thoughts and love for this language (and maybe to convince others to try it). I'll be writing more in depth about
a lot of the things mentioned here in the future.

## Syntax & Quirks

Kotlin's known for being more concise Java (and being The Android Language). It's genuinely one of it's best features, there's
...
a little overkill. That's why Kotlin doesn't suggest adding types to variables unless they're const, or properties of some
object.
...
You can also give labels to expressions, allowing clear jumping around when using nesting loops.

```kotlin
userLoop@ for (user in users) {
    for (friend in user.friends) {
        // bite all friends of a user, until the first that can't be bitten, then go to the next user
        if (!friend.biteable) continue@userLoop
        friend.bite()
    }
}
```

Lambdas can also be labelled, although usually that's unnecessary.

```kotlin
fun setupListeners() {
    ListenerRegistry.register<UserCreateEvent>() listener@{ user ->
        if (!user.activated) return@listener
        else MessageService.welcome(user)
    }
}
```
...
Extension functions are a big part of Kotlin, Ktor<sup>[2]</sup> heavily relies on them. Extension functions are when
...
am alright with them. I like that they allow lower thread usage overall, which generally helps large scale servers
like Aster run butter than if they were in Java.

Coroutines start off at a dispatcher, and then the `suspend` (async) task is sent off to an available worker. In stress
testing Aster, I've watched coroutines be able to handle quite a lot with more effective thread usage than I expected.
...
necessary to mention how much it isn't like Swift. You can read more about it [here](https://kotlinlang.org/docs/typecasts.html#smart-casts).
...
Like most programming languages, Kotlin has a mascot. Its name is Kodee, and it looks super weird, but I've come around to liking
